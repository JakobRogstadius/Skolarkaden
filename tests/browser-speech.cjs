...
(async()=>{
 ...
 input.configure({enabled:true,language:'zh-TW',lesson:'chineseTrad4'});input.getCandidates=()=>[SC.modes.chineseTrad4.items.find(i=>i.answer==='牛奶')];await input.prepare();input.start();r=sessions.at(-1);assert.equal(r.maxAlternatives,5);
 const chineseBefore=queue.length;fire(r,[Object.assign([{transcript:'錯誤'},{transcript:'妞，乃'}],{isFinal:true})]);assert.equal(queue.length,chineseBefore+1);assert.equal(queue.items.at(-1).text,'妞，乃');assert(diagnostic.some(e=>e.type==='chinese-alternative'&&e.text==='妞，乃'));input.cancel();
