...
const kinds=new Set(),orientations=new Set(),colors=new Set(),notchCounts=new Set();
for(let n=1;n<=10;n++)for(let seed=1;seed<=300;seed++){
 ...
 if(d.kind==='pie'||d.kind==='bars'){
  ...
  assert(n<=5&&d.known<=5||Math.max(n,d.known)%Math.min(n,d.known)===0,JSON.stringify(d));
  if(d.kind==='bars')orientations.add(d.horizontal);
 }else if(d.kind==='dots'){
  ...
  notchCounts.add(d.notches);assert(d.notches>=3&&d.notches<=5);assert(d.unknown>=1&&d.unknown<=d.notches);
...
assert.equal(kinds.size,4);assert.equal(orientations.size,2);assert.equal(colors.size,6);assert.equal(notchCounts.size,3);
