...
const read=file=>fs.readFileSync(path.join(__dirname,'..',file),'utf8'),dictionary=JSON.parse(read('homework.json'));
...
for(const name of ['pinyin','data','word-pairs','homework','voice','speech','input','people','game','foodtruck','plants','garden','beehive','paint','dinosaur','marshmallows','eggs','home','home-renderer'])vm.runInContext(read('resources/'+name+'.js'),ctx);
