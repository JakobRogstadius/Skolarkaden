...
test('Garden has one global 0.1-second chance trial, and math halves the probability',()=>{
 for(const pace of ['gentle','steady','brave']){
  const g=new SC.GardenGame({random:rng(4)});g.start({pace});const expected=1.3*2*.1/SC.citySpawnInterval(pace,0);assert(Math.abs(g.decayProbability()-expected)<1e-10);
...
test('Garden measured request inflow follows its 30% faster target rate',()=>{
...
test('Garden can finish with 6, 9 and 12 pots across 15 seeded gardens each',()=>{for(const pace of ['gentle','steady','brave'])for(let seed=1;seed<=15;seed++){const g=new SC.GardenGame({random:rng(seed)});g.start({pace});for(let i=0;i<6000&&['playing','celebrating'].includes(g.state);i++){if(!g.job&&!g.queue.length){const t=g.getTargets()[0];if(t)g.queue.enqueue(t.item.answer);}g.update(.05);}assert.equal(g.state,'won',pace+' seed '+seed+' at '+g.elapsed);assert(g.pots.every(p=>p.bloom||p.dead));assert(g.pots.some(p=>p.bloom));}});
