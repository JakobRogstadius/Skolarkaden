...
test('Incorrect city words cost occupancy only and pause freezes queued work',()=>{const g=new SC.CityGame();g.start();g.spawnIn=100;g.score=123;g.streak=7;g.queue.enqueue('incorrect');g.update(.05);assert.equal(g.turrets.filter(t=>t.job).length,1);tick(g,.8);assert.equal(g.shots,0);g.queue.enqueue('later');g.pause();tick(g,4);assert.equal(g.queue.length,1);g.resume();tick(g,2);assert.equal(g.score,123);assert.equal(g.streak,7);assert.equal(g.shots,2);});
...
test('Garden has one global 0.1-second chance trial, and math halves the probability',()=>{
 for(const pace of ['gentle','steady','brave']){
  const g=new SC.GardenGame({random:rng(4)});g.start({pace});const expected=2*.1/SC.citySpawnInterval(pace,0);assert(Math.abs(g.decayProbability()-expected)<1e-10);
  ...
  const ticks=[];g.onEvent=e=>{if(e.type==='care-tick')ticks.push(e);};g.random=()=>0;
  g.update(.05);assert.equal(ticks.length,0);g.update(.05);assert.equal(ticks.length,1);assert.equal(ticks[0].pots.length,1);assert.equal(g.pots.filter(p=>p.moisture<1||p.nutrition<1||p.infection>0).length,1);
...
test('Garden measured request inflow stays close to City at equal elapsed time, before hit bonuses',()=>{
...
test('Garden puts held tool down at current position, fetches needed tool and keeps it after care',()=>{const g=new SC.GardenGame({random:rng(7)});g.start();g.decayInterval=100;const p=g.pots[0];p.moisture=.4;p.nutrition=.6;g.syncRequests(p);g.gardener.x=.3;g.gardener.y=-.2;g.gardener.held=g.tools[1];g.queue.enqueue(p.requests.moisture.item.answer);g.beginJob();assert.equal(g.gardener.held,null);assert.equal(g.tools[1].x,.3);assert.equal(g.tools[1].y,-.2);assert.equal(g.job.stage,'fetch');tick(g,2);assert.equal(p.moisture,1);assert.equal(p.nutrition,.6);assert.equal(g.gardener.held.property,'moisture');p.moisture=.4;g.syncRequests(p);g.queue.enqueue(p.requests.moisture.item.answer);g.beginJob();assert.equal(g.job.stage,'run');});
...
test('Garden growth reflects care, flowers freeze needs, and fatal states lose',()=>{const g=new SC.GardenGame();g.start();g.decayInterval=100;g.pots[0].moisture=.4;tick(g,1);assert(g.pots[1].growth>g.pots[0].growth);assert(Math.abs(g.pots[1].growth-.021)<1e-8);g.pots[0].growth=.99999;g.pots[0].moisture=1;tick(g,.1);assert(g.pots[0].bloom);const moisture=g.pots[0].moisture;tick(g,7);assert.equal(g.pots[0].moisture,moisture);assert.equal(Object.keys(g.pots[0].requests).length,0);for(const key of ['moisture','nutrition','infection']){const x=new SC.GardenGame();x.start();x.pots[0][key]=key==='infection'?1:0;x.update(.05);assert.equal(x.state,'mourning');tick(x,3);assert.equal(x.state,'lost');}});
test('Garden can finish with 6, 9 and 12 pots across 15 seeded gardens each',()=>{for(const pace of ['gentle','steady','brave'])for(let seed=1;seed<=15;seed++){const g=new SC.GardenGame({random:rng(seed)});g.start({pace});for(let i=0;i<6000&&['playing','celebrating'].includes(g.state);i++){if(!g.job&&!g.queue.length){const t=g.getTargets()[0];if(t)g.queue.enqueue(t.item.answer);}g.update(.05);}assert.equal(g.state,'won',pace+' seed '+seed+' at '+g.elapsed);assert(g.pots.every(p=>p.bloom));}});
...
test('Garden health drives facial expression; victory jumps for four seconds before results',()=>{
 const events=[],g=new SC.GardenGame({onEvent:e=>events.push(e)});g.start();assert.equal(g.gardenerAnger(),0);for(const p of g.pots){p.moisture=.4;p.nutrition=.4;p.infection=.4;}assert(Math.abs(g.gardenerAnger()-(1-(.4+.4+.6)/3))<1e-8);
...
test('A dead plant stays visible for three seconds while simulation and queue freeze',()=>{
 const events=[],g=new SC.GardenGame({random:rng(8),onEvent:e=>events.push(e)});g.start();g.pots[2].nutrition=0;g.queue.enqueue('pending');g.update(.05);
