...
const ctx=vm.createContext({Event,EventTarget,CustomEvent,console});for(const f of ['pinyin','data','language-exercises-data','language-exercises','input','people','game','plants','marshmallows'])vm.runInContext(fs.readFileSync(path.join(__dirname,'../resources/'+f+'.js'),'utf8'),ctx,{filename:f+'.js'});
