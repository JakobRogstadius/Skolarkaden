...
'use strict';
require('./build-language-exercises.cjs');
const fs=require('node:fs'),path=require('node:path'),{createHash}=require('node:crypto');
const root=path.resolve(__dirname,'..');
for(const name of ['index.html','statistics.html','homework.html']){
...
if(process.argv.includes('--check')){
  if(updated!==html){console.error('Asset versions are outdated. Run npm run assets and commit '+name+' with the changed resources.');process.exitCode=1;}
}else if(updated!==html){fs.writeFileSync(file,updated);console.log('Updated asset versions in '+name+'.');}
}
