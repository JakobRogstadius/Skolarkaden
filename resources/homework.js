...
(function(root){'use strict';const SC=root.Starlight=root.Starlight||{};
...
SC.homeworkMode=base;SC.modes??={};SC.modes.homework={...base};
SC.parseHomework=function(dictionary,id){
  ...
  if(homework.name!==undefined&&(typeof homework.name!=='string'||!homework.name.trim()))invalid('name måste vara en text.');
  ...
  const items=homework.words.map((row,index)=>{
    ...
    if(chinese&&(!hint||!/^[a-z\u0300-\u036f0-5\s'’:]+$/i.test(hint.normalize('NFD'))))fail('behöver ett giltigt uttal i pinyin.');
    return {id:label,label,answer:label,speak:label,hint,translation,aliases:[]};
  });
  return {...base,homeworkId:id,homeworkName:homework.name?.trim()||id,input:homework.input,lang:homework.language,type:chinese?'chinese':'words',items};
};
SC.readHomework=async function(){
...
SC.loadHomework=async function(id,inputOverride=null){
    ...
    const lesson=SC.parseHomework(await SC.readHomework(),id);
