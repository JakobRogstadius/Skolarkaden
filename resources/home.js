...
class HomeGame{
 ...
 start({mode='swedish',pace='gentle',lang='sv-SE',items=null,uppercase=Math.random()<.5}={}){
  ...
  this.stations={
   ...
   bedsideDrawer:{type:'drawer',furniture:'bedside',x:3.65,y:5.8,anchor:{x:3.6,y:5.45,z:.4}},
  ...
  this.spots={clothes:[{x:3.6,y:7.9},{x:4.45,y:8.45},{x:7.55,y:7.55},{x:11.1,y:7.6},{x:3.2,y:10.35},{x:1.6,y:10.45},{x:8.4,y:11.0},{x:10,y:11.4}],toys:[{x:7.65,y:2.5},{x:9.5,y:3.6},{x:8.3,y:8.5},{x:9.35,y:7.55},{x:10.3,y:7.3},{x:10.1,y:1.7},{x:4.4,y:10.8},{x:2.4,y:11.25}]};
 ...
 chooseActivity(p){
  // Choose a routine, then reserve one of its free locations for the whole walk.
  // Extra floor spots must not crowd cooking and other routines out of the mix.
  const groups=new Map();for(const choice of this.activityChoices(p)){if(!groups.has(choice.type))groups.set(choice.type,[]);groups.get(choice.type).push(choice);}
 ...
 updateMeal(p,dt){
  const meal=p.meal;
  if(meal.stage==='collect'){if(this.go(p,{x:3.65,y:1.75},this.walkSpeed,dt)){meal.age+=dt;if(meal.age>=.45){meal.stage='walk';meal.age=0;this.clearRoute(p);}}return;}
  if(meal.stage==='walk'){if(this.go(p,meal.seat,this.walkSpeed,dt)){meal.stage='eat';meal.age=0;this.clearRoute(p);}return;}
  ...
  if(meal.stage==='return'){if(this.go(p,meal.bench,this.walkSpeed,dt)){meal.stage='drop';meal.age=0;this.clearRoute(p);}return;}
  meal.age+=dt;if(meal.age>=.6){this.addToStation(meal.bench.id,1,meal.kind==='snack'?'container':'plate');p.meal=null;p.wait=2*this.timeScale;this.clearRoute(p);}
 }
 updateFamily(p,dt){
  ...
  if(a.stage==='walk'){if(this.go(p,a.goal,this.walkSpeed,dt)){a.stage='use';a.age=0;p.moving=false;}return;}
  a.age+=dt;if(a.age<a.duration)return;
  if(a.type==='clothes'||a.type==='toys'){if(this.spotFree(a.goal,p))this.createTask(a.type,a.goal);}
  else if(a.type==='hungry')this.requestFood(p);
  else if(a.type==='snack'){this.addToStation('fridge');this.startMeal(p,'snack');}
  else this.addToStation(a.stationId);
